...
//^^^^^^^^^^^ premade code that i'm afraid to touch 
//vvvvvvvvvvv  my own code to make this stupid robot solve a maze using the right hand wall method B)

 void loop() {
  // Check distance in front, right, and left
  int frontDistance = watch(); //takes front distance
  delay(200);  
  head.write(0); // 0 degree = right
  delay(500); //delay so that it doesnt immediately scan the distance until its fully to the right.
  int rightDistance = watch(); //takes right distance
  delay(200); 
  head.write(180); // 180 degree = left
  delay(500); 
  int leftDistance = watch(); //takes left distance
  delay(200); 
  head.write(90); // Return "eyes" to the front


  if (rightDistance > 40) {
    // Turn right and move forward
    go_Right();
    set_Motorspeed(225, 225); //NOT CONSISTANT!!!
    delay(430); // Adjusted delay for a 90-degree turn
    go_Advance();
    delay(750);
    stop_Stop();
    ...
    go_Advance();
    delay(750);  // Adjust the delay to move forward for a specific duration
    stop_Stop();
  } else if (leftDistance > 40) {
    // Turn left and move forward
    go_Left();
    set_Motorspeed(255, 225); //for some reason, the front left wheel isnt turning when its on the ground. Increased motor speed or increase duartion of turn
    delay(500); // Adjusted delay for a 90-degree turn with the change in moterspeed
  //  set_Motorspeed(120, 120); //change motor speed back to default 120,120
    go_Advance();
    delay(750);
    stop_Stop();
  } else {
    // Rotate 180 degrees and move forward
    go_Right();
    set_Motorspeed(225, 225);  
    delay(820); //not consistant!!!
    go_Advance();
    delay(750);
    stop_Stop();
  }
